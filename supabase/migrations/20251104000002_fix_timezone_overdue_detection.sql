-- Fix overdue detection timezone handling
-- The function needs to compare times in the same timezone (local time)
...
BEGIN
  ...
  -- The task_schedule times are stored as time without time zone (local time)
  -- So we need to convert the UTC check_time to local time for comparison
  ...
  -- Debug: Log the comparison values (can be removed in production)
  -- RAISE NOTICE 'Checking date: %, time: %', v_check_date, v_check_time_only;
...
COMMENT ON FUNCTION "public"."detect_overdue_tasks_by_time" IS 'Detects tasks scheduled for today that have passed their end_time without completion. Works for both plan-based tasks (p_plan_id provided) and free-mode tasks (p_plan_id = null). Converts UTC timestamp to local timezone before comparison.';
