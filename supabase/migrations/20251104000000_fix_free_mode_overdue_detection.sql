-- Fix overdue detection to handle free-mode tasks (plan_id = null)
-- Update the function to detect overdue tasks for both plan-based and free-mode tasks
...
BEGIN
  ...
  SELECT 
    ...
    ts.duration_minutes,
    t.priority,
    NULL::integer AS complexity_score,  -- complexity_score removed, use NULL
    ts.status
  FROM "public"."tasks" t
