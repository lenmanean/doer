...
export default function ChangelogPage() {
  ...
  const descAnim = useScrollAnimation({ delay: 300, triggerOnce: true })
  const footerAnim = useScrollAnimation({ delay: 500, triggerOnce: true })
      ...
      <main className="flex-1 py-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-16 py-4">
            <div 
              ref={badgeAnim.ref as React.RefObject<HTMLDivElement>}
              className={`inline-block mb-4 scroll-animate-fade-up ${badgeAnim.isVisible ? 'visible' : ''}`}
            >
            ...
            <h1 
              ...
              className={`text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-slate-100 mb-6 bg-gradient-to-r from-slate-100 to-slate-300 bg-clip-text text-transparent leading-[1.1] py-2 scroll-animate-fade-up ${titleAnim.isVisible ? 'visible' : ''}`}
            >
              {changelogTitle}
            </h1>
            <p 
              ref={descAnim.ref as React.RefObject<HTMLParagraphElement>}
              className={`text-lg sm:text-xl text-slate-300 max-w-3xl mx-auto scroll-animate-fade-up ${descAnim.isVisible ? 'visible' : ''}`}
            >
          ...
          <div 
            ...
            className={`mt-16 text-center scroll-animate-fade-up ${footerAnim.isVisible ? 'visible' : ''}`}
          >
