...
/**
 * Get Trello connection settings
 * GET /api/integrations/trello/settings
 */
export async function GET(request: NextRequest) {
  try {
    ...
    logger.error('Failed to get Trello settings', {
...
/**
 * Update Trello connection settings
 * POST /api/integrations/trello/settings
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (updateError) {
      logger.error('Failed to update Trello settings', updateError as Error)
      return NextResponse.json(
    ...
    logger.error('Failed to update Trello settings', {
      error: error instanceof Error ? error.message : String(error),
    })
    return NextResponse.json(
      { error: 'Failed to update settings' },
