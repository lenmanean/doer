...
export async function POST(request: NextRequest) {
  try {
    ...
    if (deleteError) {
      logger.error('Failed to disconnect Todoist', deleteError as Error)
      return NextResponse.json(
    ...
    return NextResponse.json({
      success: true,
      message: 'Todoist disconnected successfully',
    })
  } catch (error) {
    logger.error('Unexpected error disconnecting Todoist', {
