...
/**
 * Get Todoist connection settings
 * GET /api/integrations/todoist/settings
 */
export async function GET(request: NextRequest) {
  try {
    ...
    logger.error('Failed to get Todoist settings', {
...
/**
 * Update Todoist connection settings
 * POST /api/integrations/todoist/settings
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (updateError) {
      logger.error('Failed to update Todoist settings', updateError as Error)
      return NextResponse.json(
    ...
    logger.error('Failed to update Todoist settings', {
      error: error instanceof Error ? error.message : String(error),
    })
    return NextResponse.json(
      { error: 'Failed to update settings' },
