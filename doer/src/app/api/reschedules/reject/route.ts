...
import { rejectRescheduleProposal } from '@/lib/task-auto-rescheduler'
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const proposalId of proposalIds) {
      try {
        const result = await rejectRescheduleProposal(supabase, proposalId, user.id)
        if (result.success) {
    ...
    if (results.length === 0) {
      return NextResponse.json({
        success: false,
        message: 'Failed to reject any proposals',
        errors
      }, { status: 500 })
    }
    ...
    return NextResponse.json({
      success: true,
      message: `Rejected ${results.length} of ${proposalIds.length} reschedule proposal(s)`,
      rejectedCount: results.length,
      errors: errors.length > 0 ? errors : undefined
    })

  } catch (error) {
    console.error('Error rejecting reschedule proposals:', error)
    return NextResponse.json(
      { 
