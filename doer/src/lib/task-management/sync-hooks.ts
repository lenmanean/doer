...
import type { TrelloProvider } from '@/lib/task-management/providers/trello-provider'
...
export async function syncTaskRescheduleToAsana(
  ...
  try {
    ...
    if (!connection || (connection as { provider?: string }).provider !== 'asana') {
      return
    }
...
export async function syncTaskCompletionToAsana(
  ...
  try {
    ...
    const connectionData = link.task_management_connections
    const connection = Array.isArray(connectionData) ? connectionData[0] : connectionData
    if (!connection || (connection as { provider?: string; auto_completion_sync?: boolean }).provider !== 'asana' || !(connection as { auto_completion_sync?: boolean }).auto_completion_sync) {
      return
    }
...
/**
 * Sync task to Trello when task schedule is updated (rescheduled)
 */
export async function syncTaskRescheduleToTrello(
  userId: string,
  taskScheduleId: string,
  newDate: string,
  newStartTime?: string | null,
  ...
  try {
    ...
    // Check if task has a Trello link
    const { data: link, error: linkError } = await supabase
      .from('task_management_links')
      .select(`
        id,
      ...
      .eq('task_schedule_id', taskScheduleId)
      .eq('task_management_connections.user_id', userId)
      .eq('task_management_connections.provider', 'trello')
      .single()

    if (linkError || !link) {
      // No Trello link exists, nothing to sync
      return
    }
    ...
    // Get provider and update task
    const provider = getProvider('trello' as TaskManagementProviderType)
    ...
    if (!result.success) {
      logger.error('Failed to sync task reschedule to Trello', {
        error: result.error,
        taskScheduleId,
        externalTaskId: link.external_task_id,
      })
    ...
    logger.error('Error syncing task reschedule to Trello', {
      error: error instanceof Error ? error.message : String(error),
      errorStack: error instanceof Error ? error.stack : undefined,
      taskScheduleId,
    })
 ...
 */
export async function syncTaskCompletionToTrello(
  userId: string,
  taskId: string,
  isCompleted: boolean
): Promise<void> {
  try {
    ...
    // Check if task has a Trello link and connection with auto_completion_sync enabled
    const { data: link, error: linkError } = await supabase
      .from('task_management_links')
      .select(`
        id,
    ...
    if (linkError || !link) {
      // No Trello link or auto_completion_sync not enabled
      return
    }
    ...
    if (isCompleted) {
      // Mark task as complete in Trello (move to Done list)
      const result = await provider.completeTask(
        connection.id,
        link.external_task_id
      )
    ...
    logger.error('Error syncing task completion to Trello', {
      error: error instanceof Error ? error.message : String(error),
      errorStack: error instanceof Error ? error.stack : undefined,
      taskId,
      isCompleted,
